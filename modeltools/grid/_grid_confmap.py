...
class ConformalMapping(object)
   ...
   def __init__(self,lat_a,lon_a,lat_b,lon_b,
      ...
      logger.info('Pole A:(%f10.4,%f10.4)'%(self._lat_a,self._lon_a))
      logger.info('Pole B:(%f10.4,%f10.4)'%(self._lat_b,self._lon_b))
      logger.info('W-E  n:(%f10.4,%f10.4,%d)'%(self._wlim,self._elim,self._ires))
      logger.info('S-N  n:(%f10.4,%f10.4,%d)'%(self._slim,self._nlim,self._jres))
      logger.info('MERC  :%7l'%self._mercator)
      logger.info('FAC   :%f14.6 %7l'%(self._mercfac,self._lold))
      ...
      self._di=(elim-wlim)/float(ires-1)   # delta lon'
      self._dj=(nlim-slim)/float(jres-1)   # delta lat' for spherical grid
      ...
      theta_a=self._lon_a*rad
      phi_a=pi_2-self._lat_a*rad
      theta_b=self._lon_b*rad
      phi_b=pi_2-self._lat_b*rad

      # find the angles of a vector pointing at a point located exactly
      # between the poles
      cx=numpy.cos(theta_a)*numpy.sin(phi_a)+numpy.cos(theta_b)*numpy.sin(phi_b)
      cy=numpy.sin(theta_a)*numpy.sin(phi_a)+numpy.sin(theta_b)*numpy.sin(phi_b)
      cz=numpy.cos(phi_a)+numpy.cos(phi_b)

      theta_c=numpy.atan2(cy,cx)
      phi_c=pi_2-atan2(cz,sqrt(cx*cx+cy*cy))

      # initialize constants used in the conformal mapping
      #imag=(.0,1.)
      imag=complex(.0,1.)
      ac=numpy.tan(.5*phi_a)*numpy.exp(imag*theta_a)
      bc=numpy.tan(.5*phi_b)*numpy.exp(imag*theta_b)
      c=numpy.tan(.5*phi_c)*numpy.exp(imag*theta_c)
      cmna=c-ac
      cmnb=c-bc
